...
public class CalendarView extends ViewPager {
    ...
    private boolean disableAfter = false;//是否禁用默认选中日期之后的所有日期
    ...
    private int[] lastClickDate = new int[2];//上次点击的日期  //上次点击的时间
    private SparseArray<HashSet<Integer>> chooseDate = new SparseArray<>();//记录多选时全部选中的日期的位子
    ...
    public void init() {
        //根据设定的日期范围计算日历的页数
        count = (dateEnd[0] - dateStart[0]) * 12 + dateEnd[1] - dateStart[1] + 1;   // 月决定viewpage的数量
     ...
     * @param position
     */
    private void refreshMonthView(int position) {  //这个position是viewpage外面的位子标记
        MonthView monthView = calendarPagerAdapter.getViews().get(position); // 获取月这个页面里面的对象
        if (itemChooseListener != null) {
            if (chooseDate.get(position) != null)  { //当月选中的数据
