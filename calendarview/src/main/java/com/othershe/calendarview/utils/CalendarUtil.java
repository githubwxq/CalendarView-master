...
public class CalendarUtil {
    ...
    public static List<DateBean> getMonthDate(int year, int month) {
        ...
        // 当月第一天是第几周 我们从0 1 2 3 4 5 6 七天开始的
        for (int i = 0; i < week; i++) {
            datas.add(initDateBean(lastYear, lastMonth, lastMonthDays - week + 1 + i, 0)); //type=0代表上月
        }

        for (int i = 0; i < currentMonthDays; i++) {
            datas.add(initDateBean(year, month, i + 1, 1));//type=1代表当月
        }

        for (int i = 0; i < 7 * getMonthRows(year, month) - currentMonthDays - week; i++) {
            datas.add(initDateBean(nextYear, nextMonth, i + 1, 2));//type=1代表下月
        }
       //配置好所有数据viewpage 每一页的数据就是这样的
        return datas;
    }
